...
def server(configpath : String?)
  ...
  RatchetExample.reticulum = RNS::ReticulumInstance.new(configpath)
  ...
  echo_destination.enable_ratchets("/tmp/#{destination_hexhash}.ratchets")
...
begin
  ...
  while i < ARGV.size
    ...
    when "-h", "--help"
      puts "Usage: ratchets [-s] [-t TIMEOUT] [--config PATH] [DESTINATION]"
      puts ""
      puts "Simple ratcheted echo server and client utility"
      puts ""
      puts "Options:"
