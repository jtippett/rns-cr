...
def client_connected(link : RNS::Link)
  ...
  link.set_remote_identified_callback(
    ->(l : RNS::Link, identity : RNS::Identity) {
      remote_identified(l, identity)
    }
  )
  IdentifyExample.latest_client_link = link
end
...
def remote_identified(link : RNS::Link, identity : RNS::Identity)
  RNS.log("Remote identified as: " + identity.to_s)
end
...
def server_packet_received(message : Bytes, packet : RNS::Packet)
  ...
  if link = IdentifyExample.latest_client_link
    if ri = link.get_remote_identity
      remote_peer = ri.to_s
    end
  end
  ...
  if client_link = IdentifyExample.latest_client_link
...
def client(destination_hexhash : String, configpath : String?)
  ...
  IdentifyExample.client_identity = RNS::Identity.new
  RNS.log("Client created new identity " + IdentifyExample.client_identity.to_s)
...
def client_loop
  ...
  while IdentifyExample.server_link.nil?
    sleep 0.1
  end
  ...
  while !should_quit
    begin
      ...
      if text.nil?
        should_quit = true
        IdentifyExample.server_link.try &.teardown
        next
      end
      ...
      if text != "" && !should_quit
        ...
        if data.size <= RNS::Link::MDU
          if link = IdentifyExample.server_link
...
begin
  ...
  while i < ARGV.size
    ...
    when "-h", "--help"
      puts "Usage: identify [-s] [--config PATH] [DESTINATION]"
      puts ""
      puts "Simple link identification example"
      puts ""
      puts "Options:"
